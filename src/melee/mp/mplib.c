...
void mpLibLoad(MapCollData* coll_data)
{
    ...
    mp_pc_real_coll = 1;
#endif
    /* This used to be HSD_MemAlloc(0x3400): a GameCube *byte* count, which is
     * exactly 256 CollJoints there and matches the joint_id < 256 bound this
     * file checks (mpLib_80057D9C). A CollJoint holds seven pointers, so it
     * grew from 0x34 to 0x50 bytes here and the same 0x3400 covered only 166
     * joints. Every stage with more collision joints than that ran off the
     * end -- mpLib_80057BC0 and mpJointSetCb1/Cb2 index groundCollJoint
     * directly -- which is where nine stages were dying. groundCollJoint_count
     * is that same 256, counted in entries. */
    groundCollJoint =
    ...
    if (getenv("MELEE_MPDUMP") != NULL) {
        ...
        fprintf(stderr, "[MPDUMP] verts=%d lines=%d joints=%d floors=%d..+%d "
                ...
                coll_data->joint_count,
                coll_data->ranges[MapLineGroup_Floor].start,
                coll_data->ranges[MapLineGroup_Floor].count,
                coll_data->ranges[MapLineGroup_Ceiling].start,
                coll_data->ranges[MapLineGroup_Ceiling].count,
                coll_data->ranges[MapLineGroup_RightWall].start,
                coll_data->ranges[MapLineGroup_RightWall].count,
                coll_data->ranges[MapLineGroup_LeftWall].start,
                coll_data->ranges[MapLineGroup_LeftWall].count,
                coll_data->ranges[MapLineGroup_Dynamic].start,
                coll_data->ranges[MapLineGroup_Dynamic].count, (double) f31);
        for (jn = 0; jn < coll_data->joint_count; jn++) {
            ...
            fprintf(stderr, "[MPDUMP] joint %d: floor=%d+%d ceil=%d+%d rw=%d+%d "
                    ...
                    jn, mj->ranges[MapLineGroup_Floor].start, mj->ranges[MapLineGroup_Floor].count, mj->ranges[MapLineGroup_Ceiling].start,
                    mj->ranges[MapLineGroup_Ceiling].count, mj->ranges[MapLineGroup_RightWall].start,
                    mj->ranges[MapLineGroup_RightWall].count, mj->ranges[MapLineGroup_LeftWall].start,
                    mj->ranges[MapLineGroup_LeftWall].count, mj->ranges[MapLineGroup_Dynamic].start, mj->ranges[MapLineGroup_Dynamic].count,
                  ...
                  Fighter_GObj* gobj)
{
    ...
    MP_JOINT_WALK(joint) {
        ...
        block_8:
            ...
            y1_sp3C += y_offset;
#if BUILD_TARGET_PC
            /* line_offset is a byte offset, not a line id: divide before
             * comparing (upstream now spells the stride sizeof(CollLine),
             * which is 16 here and 8 on the console). */
            if (getenv("MELEE_FLOORTRACE") != NULL &&
                line_offset / (ssize_t) sizeof(CollLine) == 24)
            {
                static int n2 = 0;
                if (n2 < 200) { n2++;
...
/* The eight walkers below step the line array by byte offset from its own
 * address: ((CollLine*) ((int) groundCollLine + id * sizeof(CollLine)))->x0.
 * The (int) truncates a 64-bit pointer here, and the array is well above
 * 4 GB on Android, so read it as an array instead. The offset is still
 * counted in whatever a CollLine measures on this target, so dividing it by
 * sizeof(CollLine) gives the line id back; the console expansion is the
 * expression upstream wrote, token for token. */
#if BUILD_TARGET_PC
#define MP_LINE_AT(off) (&groundCollLine[(off) / (int) sizeof(CollLine)])
#else
#define MP_LINE_AT(off) ((CollLine*) ((int) groundCollLine + (off)))
#endif
...
again: {
    ...
    CollVtx* vtx =
        &groundCollVtx[MP_LINE_AT(line_offset)->x0->v1_idx];
    pos_out->x = vtx->pos.x;
    pos_out->y = vtx->pos.y;
    pos_out->z = 0.0F;
}
...
again: {
    ...
    CollVtx* vtx =
        &groundCollVtx[MP_LINE_AT(line_offset)->x0->v0_idx];
    pos_out->x = vtx->pos.x;
    pos_out->y = vtx->pos.y;
    pos_out->z = 0.0F;
}
                    ...
                    u32* flags_out, Vec3* normal_out, f32 var_f25, f32 arg7)
{
    ...
    float sqrt_tmp_store[7];
    float* const sqrt_tmp = sqrt_tmp_store + 5;
#else
    float sqrt_tmp[2];
#endif
    float dist_f28;

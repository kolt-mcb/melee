...
{
    u32 y_tex = (u32) RF_FMA(127.0f, y * param0, 128.0f);
...
static inline float my_fmodf(float a, float b)
{
    ...
    /* fnmsubs on the console: the product and the subtract share one
     * rounding, and the truncation goes through u64 the way the DOL does. */
    quotient = (s64) (u64) (a / b);
    return RF_FMA(-b, (float) quotient, a);
}
...
static void lbRefract_80021CE8(lbRefract_CallbackData* cb, s32 arg1)
{
    ...
    for (row = 0; row < cb->height; row++) {
        ...
        for (col = 0; col < cb->width; col++) {
            /* Not sqrtf: the console takes frsqrte and three Newton steps in
             * double, which is a couple of ulps off a correctly-rounded
             * square root. The sum is fused (fmadds). */
            f32 dist_sq = RF_FMA(x, x, y_sq);
            f32 dist;
            if (dist_sq > 0.0f) {
                f32 sqrt_dist;
...
#if BUILD_TARGET_PC
    /* PC port: refraction stays off here, as it has been all along.
     *
     * It used to be off by accident -- the descriptors were zero-filled DOL
     * data, the symbol load failed, and everything below dereferenced NULL.
     * Upstream has since decompiled the real descriptors, so the load now
     * succeeds and the code below runs for the first time. It cannot run yet:
     * lbRefData is raw big-endian DAT bytes, and the reader treats them as a
     * host structure -- `*(f32**)(refract_data + 4)` takes eight bytes
     * spanning two of the file's four-byte fields as a pointer, which is the
     * address it crashed on (0x1c00000000). The count at offset 0 is read as
     * a single byte of a big-endian word as well.
     *
     * Turning it on means converting the descriptor the way the other DAT
     * tables are converted. Until then this keeps the port's behaviour
     * exactly as it was before the merge rather than crashing on entry to a
     * versus match. */
    PORT_LOG_WARN("lbRefract_800222A4: refraction skipped (lbRefData is not "
                  "converted to host layout yet)\n");
    return;
#endif
    {
        size_t buf_size =

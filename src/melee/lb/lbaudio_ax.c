    ...
    if (slot != -1) {
        ...
        if (getenv("MELEE_AXTRACE")) {
            fprintf(stderr,
                    ...
                    "req %d total %d)\n",
                    slot, cur_ssm_file, offsets_arr_803BC4E4[slot][0],
...
{
#if BUILD_TARGET_PC
    /* 0x48 was the console's sizeof(lbAudioAx_UserData) and upstream now
     * writes sizeof too. The host struct is 0x60: four of its members are
     * pointers (gobj, entity, the x10 callback and the x2C union), and each is
     * twice as wide here, so a fixed 0x48 meant every sound object wrote 24
     * bytes past its slot -- over the next object in this pool, and, for the
     * last object in a pool chunk, over whatever the heap put after it.
     * Staryu's throw sound put `arg8` (60) and `f1` (3.0f) straight through a
     * live HSD_GObjProc's `next` pointer and the gobj walk faulted on it a
     * frame later. The alignment has to be 8 here for the same reason. */
    HSD_ObjAllocInit(&lbl_80433710, sizeof(lbAudioAx_UserData), 8);
#else

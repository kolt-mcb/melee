...
struct CameraBlurData {
    ...
    /* PC port: efb_copy has to be a pointer in every view of this struct --
     * the setup path (lb_800138EC) writes it here while the render path in
     * lbspdisplay.c reads it. As an s32 the write truncated a 64-bit pointer
     * to its low half. Upstream now types it as HSD_ImageDesc* for every
     * target, so the old BUILD_TARGET_PC override here is no longer needed. */

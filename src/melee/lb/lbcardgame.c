...
#if BUILD_TARGET_PC
    /* The memory-card icon archive never loads on this port (see
     * lbCardGame_LoadArchive), so icon_data stays null and the save-banner
     * path dereferenced it during scene teardown. */
    if (!pc_ptr_sane(_p(icon_data))) {
        return 0;
    }
#endif
...
void lb_8001CC84(void)
{
     ...
     * machine below reads _p(icon_data)[...] for the save banner. Retire the

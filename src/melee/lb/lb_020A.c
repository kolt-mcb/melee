 ...
 * leg bones carry the difference into the AI hash. (These functions lived in
 * lbbgflash.c before upstream split them out into this file.) */
#if BUILD_TARGET_PC
#include <math.h>
...
static inline f32 sqrtf_store(f32 x, volatile f32* y)
{
    if (x > 0.0f) {
        ...
        guess = 0.5 * guess * BF_FMAD(-(f64) x, guess * guess, 3.0);
        guess = 0.5 * guess * BF_FMAD(-(f64) x, guess * guess, 3.0);
        guess = 0.5 * guess * BF_FMAD(-(f64) x, guess * guess, 3.0);
        *y = (f32) (x * guess);
        return *y;
...
void lbBgFlash_80021410(IKState* data)
{
    ...
    sin_val = sqrtf_store(BF_FMA(-dot, dot, 1.0f), &sin_mag);

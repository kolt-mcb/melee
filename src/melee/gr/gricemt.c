 ...
 * 0xB0, 0xB4 as archive offsets, and half-word pairs elsewhere.
 * Ground_GetYakumonoParam cannot express that (host pointers are eight
 * bytes, so the struct's own offsets stop matching at 0xAC), so this file
 * converts its block itself, field by field, into a host struct, and copies
 * the three tables out of the archive byte-swapped. Without this every read
 * of the block -- the initial scroll height that puts both spawn points
 * 18.67 above the console's, the segment ids at field_ixs[0..4], the spawn
 * table -- was reading swapped bytes. */
...
static struct grIceMt_YakumonoParam* pc_icemt_param(const void* raw)
{
    ...
    /* 0x00..0xAB: 32-bit scalars, except the half-word fields. The host
     * struct has the same offsets up to here. */
    for (o = 0; o < 0xAC; o += 4) {
        /* Seven of the words are pairs of s16 -- 0x00 (1,3), 0x04, 0x34
         * (60,360), 0x38 (900,30), 0x98 (48,70), 0xA4 (15,11), 0xA8
         * (-190,0) in GrIm.dat -- whatever the struct calls them. A word
         * swap crossed each pair: the scroll-speed threshold at 0x98 and the
         * timer at 0xA4 came out as the other half, and the stage scrolled
         * at a fifth of the console's speed. Keep those halves at their
         * console bytes. */
        if (o == 0x00 || o == 0x04 || o == 0x34 || o == 0x38 || o == 0x98 ||
            o == 0xA4 || o == 0xA8)
        {
...
void stageGObj10_GObjProc(Ground_GObj* arg0)
{
    ...
    if (!gp->u.icemt10.x14_b2) {
        ...
        if (dist < 0.0f) {
            ...
            if (IC_FMA(gp->u.icemt10.x0.state.cur, ratio,
                       ...
                       float y)
{
     ...
     * there simply does not scroll. The same guard exists a few functions
     * down in grIceMt_801F9ACC. */
    if (arg0->under != -1) {
        HSD_JObj* jobj;
...
float grIceMt_801F993C(int under_id, int upper_id)
{
    ...
    return IC_FMA(-y, icemt_field[upper_ix].x8,
                  IC_FMA(y, icemt_field[under_ix].x4, yakumono_param->x40));
}

#if BUILD_TARGET_PC
/* Icicle Mountain looks its segments up by id and not all of them exist on
 * PC yet; Ground_GetMapGObj then returns NULL and GET_GROUND of it is a
 * dereference of NULL+user_data. */
static Ground* pc_icemt_seg_gp(int id)
{
    HSD_GObj* g = Ground_GetMapGObj(id);
    return pc_ptr_sane(g) ? (Ground*) g->user_data : NULL;
}
#define ICEMT_SEG_GP(id)                                                      \
    pc_icemt_seg_gp(id);                                                      \
    if (gp == NULL) {                                                         \
        port_guard_warn("gricemt.c:no-segment-gp");                           \
        return result;                                                        \
    }
#else
#define ICEMT_SEG_GP(id) GET_GROUND(Ground_GetMapGObj(id))
#endif
                      ...
                      float y, GrIceMtSegmentLookup ev, Ground_GObj* arg3)
{
    ...
    cur = HSD_JObjGetTranslationY(jobj);
    if (ABS(cur) < 10.0f) {
        gp = ICEMT_SEG_GP(ids->upper);
        gp->u.icemt1.x0_b1 = true;
    } else if (ABS(cur + y0) < 10.0f) {
        gp = ICEMT_SEG_GP(ids->under);
        gp->u.icemt1.x0_b1 = true;
    }
    if (cur < 0.5f * -y0) {
        id = ids->under;

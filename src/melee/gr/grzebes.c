 ...
 * index}; upstream types it that way now too. The console stored it as u32
 * words, whose big-endian bytes are exactly that s16 stream; read as words on
 * a little-endian host each pair of halves came out swapped, so one wrong
 * joint got linked and the main floor (joint 4, map 6, its 14th descendant)
 * stayed in joint-local space around the origin -- fighters spawned at the top
 * and fell to the bottom line. */
...
void grZebes_801D8644(HSD_GObj* gobj)
{
    ...
    gp->u.zebes5.xF0 = (uintptr_t) grZebes_801D8558(7);
#else
    ...
    gp->u.zebes5.x100 = (uintptr_t) mat_gobj2;
    grZebes_801DC9DC((s32) gobj);
    gp->u.zebes5.xFC = (uintptr_t) grZakoGenerator_801CA394(
...
void grZebes_801D881C(HSD_GObj* gobj)
{
    ...
    f32 sp14;
    Ground* gp = GET_GROUND(gobj);
    HSD_GObj* secondary_gobj = (HSD_GObj*) gp->u.zebes5.xF0;
    s32 result = grZebes_801DA528(gobj, &gp->u.zebes5.xC8, 1, 2);
    PAD_STACK(8);
...

    {
        ...
        if (timer < 0) {
            grZebes_801DAA08();
            {
                ...
                gp->u.zebes5.xF8 = (s16) ZB_FMA(diff, rand, base); /* 801D8910 */
            }
        }
    }

    {
        ...
        case 1:
            if (grAnime_801C83D0(gobj, 0xE, 1) != 0) {
                ...
                gp->u.zebes5.xC6 = (s16) ZB_FMA(diff, rand, base); /* 801D8A10 */
            }
            break;
        case 2:
            ...
            if (eq_counter % divisor == 0) {
                ...
                if (spawn_phase < mirror) {
                    ...
                    grZebes_801DAE70(spawn_phase, 4, *(f32*) (base + 0x14),
                                     *(f32*) (base + 0x18),
#endif
                                     ZB_FMA(yakumono_param->x5C - scale_min,
                                            rand, scale_min));
                }
                if (spawn_phase <= mirror) {
#if BUILD_TARGET_PC
...
void grZebes_801D9100(HSD_GObj* gobj)
{
    ...
    gp->u.zebes4.xD8 = PC_GVPTR(child_jobj);
    new_var = 0xD;
    gp->u.zebes4.xDC = PC_GVPTR(Ground_801C3FA4(gobj, 0x11));
    gp->u.zebes4.xE0 = PC_GVPTR(mat_gobj);
    gp->u.zebes4.xE4 = new_var;
    gp->u.zebes4.xE8 = 0;
    gp->u.zebes4.xEC = (uintptr_t) grZakoGenerator_801CA394(
...
void grZebes_801D9508(Ground_GObj* gobj)
{
    ...
    gp->u.zebes.x4 = (uintptr_t) Ground_801C3FA4(map_a_gobj, 14);
    gp->u.zebes.x8 = 1;
...
void grZebes_801D9798(HSD_GObj* gobj)
{
    ...
    rand = HSD_Randf();
    gp->u.zebes5.xD8 = ZB_FMA(yakumono_param->x94, rand,
...
void grZebes_801D99E0(HSD_GObj* gobj)
{
    ...
    case 1: {
        ...
        if (gp->u.zebes5.xC6 < 0) {
            ...
            diff = gp->u.zebes5.xD4;
            diff -= gp->u.zebes5.xD8;
            if (diff < 0.0f) {
                diff = -diff;
            }
    ...
    case 3: {
        ...
        t = vel / accel;
        delta = gp->u.zebes5.xD4 - gp->u.zebes5.xD8;
        ...
        if (abs_delta < threshold || ((delta < 0.0f) ? -delta : delta) < accel)
        {
            ...
            gp->u.zebes5.xD0 = gp->u.zebes5.xD0 + accel;
        ...
        {
            f32 cur = gp->u.zebes5.xD8;
            if (gp->u.zebes5.xD4 > cur) {
                gp->u.zebes5.xD8 = cur + gp->u.zebes5.xD0;
            } else {
                gp->u.zebes5.xD8 = cur - gp->u.zebes5.xD0;
            }
        }
    ...
    state4_done:
        ...
        gp->u.zebes5.xC4 = (s16) (gp->u.zebes5.xC4 + 1);
        {
            ...
            if (idx == 0x1E) {
                gp->u.zebes5.xC4 = 0;
            } else if (yakumono_param->xA0_entries[idx].x0_base == 0 &&
...
s32 grZebes_801DA528(HSD_GObj* arg0, void* arg1, s32 arg2, s32 arg3)
{
    ...
    if (st->x00_state != st->x01_next) {
        ...
        case 2: {
            ...
            st->x02_timer = (s16) ZB_FMA(f0, f1, yakumono_param->x50); /* 801DA5F8 */
            break;
        }
        case 3:
...
#if BUILD_TARGET_PC
/* The acid's attack parameters. yakumono_param is the stage's yakumono block
 ...
 *
 * The offset has to be resolved against the *raw* block: Ground_GetYakumonoParam
...
bool grZebes_801DCBFC(Ground_GObj* gobj, HSD_GObj* fobj, void* arg)
{
    ...
    if (pos.y < slope) {
#if BUILD_TARGET_PC
        /* `((HSD_GObj*) yakumono_param)->user_data` is a decompiler artifact
         * for "load the word at +0x2C": yakumono_param is a flat scalar block
        ...
        {
            u32 slot = *(const u32*) ((const u8*) yakumono_param + 0x2C);

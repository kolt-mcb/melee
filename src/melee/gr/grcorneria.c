...
/* Fused on the console (fmadds/fmsubs/fnmsubs/fnmadds); pairing read off the
 * DOL. */
#if BUILD_TARGET_PC
#include <math.h>
...
void grCorneria_801DD9A8(Ground_GObj* gobj)
{
    ...
    if (!gp->u.corneria.xC4.flags.b0) {
        ...
        speed = CN_FMA(-35.0f, Ground_801C0498(),
                       -gp->u.corneria.xD0); /* fmsubs */
    ...
    if (arwing_gobjs[gp->u.corneria2.xC8] != NULL) {
        ...
        case 13: {
                ...
                gp->u.corneria2.xFC != 0)
            {
                ...
                grMaterial_801C9604(gobj,
                                    GR_COLOR_SCRIPT(yakumono_param->x84), 0);
            }
        }
        }
...
UNK_T grCorneria_801E03C8(Ground_GObj* gobj, int id)
{
    ...
    if (id == 8) {
        ...
        HSD_JObjSetTranslateX(HSD_GObjGetHSDObj(new_gobj),
                              x - CN_FMA(4800.0f * Ground_801C0498(), 0.5f,
                                         half));
        gp->u.corneria.xC5 = 0;
    } else if (id == 9) {
        f32 half = 3200.0f * Ground_801C0498() / 2;
        HSD_JObjSetTranslateX(GET_JOBJ(new_gobj),
                              x - CN_FMA(3200.0f * Ground_801C0498(), 0.5f,
                                         half));
        gp->u.corneria.xC5 = 0;
    } else if (id == 4) {
...
void grCorneria_801E0678(void)
{
    ...
    gp->u.corneria.xC6.flags.b0 = 1;
    {
        ...
        HSD_JObjSetTranslateX(gobj->hsd_obj,
                              -CN_FMA(3200.0f * Ground_801C0498(), 0.5f,
                                      half) /* fnmadds */);
    }
...
void grCorneria_801E1970(Ground_GObj* gobj)
{
    ...
    if (gp->u.corneria.x104 <= 0) {
        ...
        gp->u.corneria.x104 = CN_FMA(a, randf, yakumono_param->x0);
        randf = HSD_Randf();
...
void grCorneria_801E2228(Ground_GObj* gobj)
{
    ...
    if (!gp->u.corneria.xC4.flags.b1) {
        ...
        if (timer < 0x384) {
            ...
            f32 speed = gp->u.corneria.xFC;
            f32 t = speed / 0.005f;
            if (-gp->u.corneria.xD0 > CN_FMA(speed, t, -0.0025f * t * t)) {
                t = 0.005f;
...
#if BUILD_TARGET_PC
            /* A SIS block straight out of the archive is big-endian and its
             * internal references are file offsets; convert it before the
             * text layer walks it. */
...
f32 grCorneria_801E2FCC(void)
{
    ...
    if (gobj != NULL) {
        gp = gobj->user_data;
        if (gp != NULL) {
            return CN_FMA(Ground_801C0498(), -35.0f,
                          -gp->u.corneria.xD0) +
                   5.0f;
        }
    }

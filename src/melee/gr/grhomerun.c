...
#if BUILD_TARGET_PC
    /* The port's asserts are non-fatal, so the two loops below would walk a
     * NULL array. (The allocations themselves used to be a flat 0x100 bytes,
     * a GameCube byte count for 64 four-byte pointers; the sizeof spelling
     * above is what makes them right here.) */
    if (gp->u.homerun.parts == NULL || gp->u.homerun.back == NULL) {
        port_guard_warn("grhomerun.c:no-parts");
        return;
    }
#endif

...
HSD_LObj* grShrineRoute_8020AB58(Ground_GObj* arg);
HSD_LObj* grShrineRoute_8020AC44(HSD_GObj* arg);
...
{
    int val;
    grZakoGenerator_801CAE04(&grSh_Route_804D6A58[0]->x28);
    val = grSh_Route_804D6A58[0]->x24;
    if (val != 0) {
        val = HSD_Randi(grSh_Route_804D6A58[0]->x24);
    } else {
        val = 0;
    }
    if (val == 0) {
...
void fn_80208A38(HSD_GObj* gobj)
{
    ...
    for (i = 0; i < 6; flag++, i++) {
        ...
        if (jobj != NULL) {
            if (*flag != 0) {
                gp->gv.shrineroute2.xC4.arr[i] = grShrineRoute_802088C0(3);
            } else {
                gp->gv.shrineroute2.xC4.arr[i] = grShrineRoute_802088C0(1);
            }
            lb_8000B1CC(jobj, NULL, &pos);
            effect = gp->gv.shrineroute2.xC4.arr[i];
            if (effect != NULL) {
                if ((ejobj = GET_JOBJ(effect)) != NULL) {
                    HSD_JObjSetTranslate(ejobj, &pos);
                    HSD_JObjSetScaleX(ejobj, grSh_Route_804D6A58[0]->x14);
                    HSD_JObjSetScaleY(ejobj, grSh_Route_804D6A58[0]->x14);
                    HSD_JObjSetScaleZ(ejobj, grSh_Route_804D6A58[0]->x14);
                }
            }
        }
    }
...
void grShrineRoute_80208D14(Ground_GObj* gobj)
{
    ...
    for (i = 0; i < 6; i++) {
        gp->gv.shrineroute2.xC4.arr[i] = NULL;
    }

    ftCo_800C07F8(gobj, 3, grShrineRoute_8020AE08);
    mpJointSetCb1(8, gp, (mpColl_Callback) (Event) grShrineRoute_8020AD58);
    mpJointSetCb1(9, gp, (mpColl_Callback) (Event) grShrineRoute_8020AD58);
    mpJointSetCb1(0xA, gp, (mpColl_Callback) (Event) grShrineRoute_8020AD58);
    mpJointSetCb1(0xB, gp, (mpColl_Callback) (Event) grShrineRoute_8020AD58);
    mpJointSetCb1(0xC, gp, (mpColl_Callback) (Event) grShrineRoute_8020AD58);
    mpJointSetCb1(0xD, gp, (mpColl_Callback) (Event) grShrineRoute_8020AD58);
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (jobj != NULL) {
                HSD_JObjGetTranslation(
                    gp->gv.shrineroute.platforms[i].jobj,
...
void grShrineRoute_80208F70(Ground_GObj* gobj)
{
    ...
    case 0: {
        ...
        s32 ix;
        track_plat = 0;
        radius = grSh_Route_804D6A58[0]->x20 * Ground_801C0498();
        result = Ground_801C3DB4(
            grShrineRoute_80208F14,
            grSh_Route_804D6A58[0]->x1C * Ground_801C0498(), radius);
        if (result != -1) {
            ix = result - 0xBD;
            if (!(gp->gv.shrineroute.xC6 & (1 << ix))) {
                gp->gv.shrineroute.xC8 = (u16) result;
                HSD_ASSERTMSG(0x213, (HSD_GObj**) &gp->gv.shrineroute2.xC4.ptr,
                              "gp->u.map.symbol[ix]");
                {
                    s32 mid =
                        ((Ground*) ((HSD_GObj**) &gp->gv.shrineroute2.xC4.ptr)[ix]->user_data)
                            ->map_id;
                    if (player != NULL) {
                    ...
                    if (mid == 3) {
                        ...
                        grShrineRoute_8020B020(
                            (HSD_GObj*) gobj, (s32) gp->gv.shrineroute.xC8, 1);
                    }
                }
            }
        }
    ...
    case 2: {
        ...
        }
        {
                ...
                (gp->gv.shrineroute.xD4 == 0))
            {
                ...
                Ground_801C5750();
                gm_801674C4(6, 1, 1, (s8) (gp->gv.shrineroute.xC8 - 0xA),
    ...
    case 3: {
        track_plat = 1;
        if ((Ground_801C5764() == 1) && (gp->gv.shrineroute.xCA != 0) &&
            (gp->gv.shrineroute.xCC != 0))
        {
            gp->gv.shrineroute.xC6 =
                (u16) (gp->gv.shrineroute.xC6 |
...

    {
        ...
        if (timer == 0) {
            ...
            gp->gv.shrineroute.xCE = (u16) (gp->gv.shrineroute.xCE - 1);
        }
    }
...
void grShrineRoute_8020A104(Ground_GObj* gobj)
{
    ...
    gp->gv.shrineroute2.xC4.ptr = lgobj;
    grShrineRoute_8020AA40((HSD_GObj*) gp->gv.shrineroute2.xC4.ptr);

    lobj = GET_LOBJ((HSD_GObj*) gp->gv.shrineroute2.xC4.ptr);
    ...
    gp->gv.shrineroute2.x16C = grShrineRoute_8020AB58(gp->gv.shrineroute2.xC4.ptr);
    gp->gv.shrineroute2.x170 = grShrineRoute_8020AC44(gp->gv.shrineroute2.xC4.ptr);
...
void grShrineRoute_8020A21C(Ground_GObj* gobj)
{
    ...
    if ((u32) gp->gv.shrineroute2.xC4.ptr != 0) {
        sp = sorted;
        sp2 = sp;
        dp = distances;
        i = 0;
        ...
        while (i < (s32) gp->gv.shrineroute2.x168) {
            if (i < 6) {
                HSD_LObjClearFlags(gp->gv.shrineroute2.xC8[*sp], 0x20);
            } else {
                HSD_LObjSetFlags(gp->gv.shrineroute2.xC8[*sp], 0x20);
            }
            sp += 1;
            i += 1;
        }
        ...
        i = 1;
        while (i < (s32) gp->gv.shrineroute2.x168) {
            if (HSD_LObjGetPosition(gp->gv.shrineroute2.xC8[*sp],
                                    &light_pos) != 0)
            {
        ...
        if (HSD_LObjGetInterest(gp->gv.shrineroute2.xC8[sorted[1]],
                                &temp_pos) == 0)
        {
            temp_pos.z = 0.0f;
            temp_pos.y = 0.0f;
...
void grShrineRoute_8020AF38(HSD_GObj* gobj, s32 arg1)
{
    ...
    if (gp->gv.shrineroute2.xC4.arr[ix] != NULL) {
        symbolp = gp->gv.shrineroute2.xC4.arr + ix;
        scale = 0.7f;
        if (((Ground*) gp->gv.shrineroute2.xC4.arr[ix]->user_data)->map_id == 1)
        {
            scale *= grSh_Route_804D6A58[0]->x14;
        } else {
            scale *= grSh_Route_804D6A58[0]->x18;
        }
        jobj = Ground_801C2CF4(arg1);
        efSync_Spawn(0x428, gobj, jobj, &scale);
        Ground_801C4A08(gp->gv.shrineroute2.xC4.arr[ix]);
        *symbolp = NULL;
        if (pgobj != NULL) {
...
void grShrineRoute_8020B020(HSD_GObj* gobj, int r4, int r5)
{
    ...
    for (i = 0; i < 6; i++) {
        if (r4 == -1 || i != comp) {
            if (gp->gv.shrineroute2.xC4.arr[i]) {
                if (GET_JOBJ(gp->gv.shrineroute2.xC4.arr[i])) {
                    if (r5) {
                        HSD_JObjSetFlagsAll(
                            gp->gv.shrineroute2.xC4.arr[i]->hsd_obj,
                            JOBJ_HIDDEN);
                    } else {

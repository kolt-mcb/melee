...
#if BUILD_TARGET_PC
    /* Indexing the callback table with an out-of-range id yields a pointer
     * that passes a sanity check and faults on first use; refuse it here. */
    if (gobj_id < 0 ||
        (unsigned) gobj_id >=
            sizeof(stage_callbacks) / sizeof(stage_callbacks[0]))
    {
        port_guard_warn("grpura.c:bad-gobj-id");
        return NULL;
    }
#endif
    callbacks = &stage_callbacks[gobj_id];
...
void stageGObj1_GObjProc(Ground_GObj* arg0)
{
    ...
    if (gp->u.pura.xC8 < 0xE10) {
        ...
        gp->u.pura.xC8 = cur + 1;
        sp18.r =
            (s8) PU_FMA(t, (f32) (spilC.r - sp18.r), (f32) sp18.r);
        sp18.g =
            (s8) PU_FMA(t, (f32) (spilC.g - sp18.g), (f32) sp18.g);
        sp18.b =
            (s8) PU_FMA(t, (f32) (spilC.b - sp18.b), (f32) sp18.b);
...
#if BUILD_TARGET_PC
            /* The port's asserts are non-fatal, so do not dereference. */
            if (gobj == NULL) {
                continue;
            }
#endif

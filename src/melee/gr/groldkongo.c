    ...
    if (timer > 0) {
        ...
        if (gp->u.unk.xC4 == 0) {
            ...
            HSD_JObjSetTranslateY(
                jobj, OK_FMA(yakumono_param->rrange_bird_random_offset_y,
                             OK_FMA(2.0f, (0, HSD_Randf()), -1.0f), 70.0f));
...
bool grOldKongo_80210454(Ground_GObj* ground_gobj, Fighter_GObj* keep)
{
    ...
    gp->u.taru.xCA =
        (s16) OK_FMA(diff, rand_val, yakumono_param->rframe_barrel_shoot_a);
    gp->u.taru.keep = keep;
    gp->u.taru.xC6 = 1;
    Ground_801C5440(gp, 0, 0x129U);
    grAnime_801C7FF8(ground_gobj, 2, 7, 1, 0.0f, 1.0f);
    grMaterial_801C9604(ground_gobj, GR_COLOR_SCRIPT(yakumono_param->x6C), 0);

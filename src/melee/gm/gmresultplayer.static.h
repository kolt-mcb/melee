...
#if BUILD_TARGET_PC
    /* PC port: on the console lbl_8046E38C, lbl_8046E39C and lbl_8046E3AC are
     * the three .bss objects that follow lbl_8046E1B0, and the code reaches
     * them both by name and by casting lbl_8046E1B0 to ResultsDisplayLayout.
     * Separate objects here made a write through one name invisible through
     * the other -- fn_80179F6C stored the per-player GObjs where nothing read
     * them, and `disp->gobjs[slot]` was NULL at the end of every match with a
     * placement box to draw. Carry the three as members, and alias the names
     * onto them below, so there is one object again; the offsets no longer
     * match the console's, which nothing here depends on. */

...
void fn_8017FF1C(HSD_GObj* gobj)
{
    ...
    if (data.state->x117 != 0 && data.state->x110 > 0x29U) {
        data.state->xC0 =
            fn_8017F47C(data.state->x84, data.state->xA4, (s32) data.state->xC0);
...
void fn_80180630(int arg0, int arg1, int arg2, bool arg3, MatchEnd* arg4)
{
    ...
    if (getenv("MELEE_1P_CPU") != NULL) {
        Player_GetPtrForSlot(0)->pkind = Gm_PKind_Human;
    }
#endif
    /* 0x120 was the GameCube size of this struct; every pointer in it is
     * twice as wide here, so the literal cleared only the front of the object
     * and left everything past it -- the HSD_Text array among them -- holding
     * whatever the previous scene had put there. */
...
#if BUILD_TARGET_PC
        /* PC port: lbArchive_80016DBC hands back raw big-endian archive
         * data. fn_80168A6C flattens the SceneDesc and its models[idx] into
         * a run of 32-bit words, which is only a layout on GameCube -- on
         * x86_64 every one of those words is half a pointer, and src->x0[0]
         * faulted on the first read. Convert the scene and copy the same
         * four things out by name. */
        {
            SceneDesc* scene = pc_conv_SceneDesc(
                scene_data, archive != NULL ? archive->data : NULL);
            if (scene == NULL) {
                PORT_LOG_WARN("fn_80180630: GmRegClr scene data would not "
                              "convert; clear screen skipped\n");
                return;
            }
            memzero(fn_80180630_GetModelDesc(state),
                    sizeof(state->x4C));
            ...
            state->x60 =
                scene->cameras != NULL ? scene->cameras[0].desc : NULL;
        }
#else

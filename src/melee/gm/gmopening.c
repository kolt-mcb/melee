...
void gm_Scene_Opening_OnEnter(UNK_T arg0)
{
    ...
    /* PC port: lbMthp_8001F410 now opens the real movie (src/port/pc_mth.c).
     * It degrades to advancing the 60Hz counter alone if MvOpen.mth is
     * missing or MELEE_NO_MOVIE is set, which is what the port did before --
     * the title sequence below is driven by that counter, not by the movie. */
    gm_804D67EC = 0;

 ...
 * 0x78 even on the disc; on the host it is larger again, because its ten
 * callback slots are eight bytes each rather than four. Reading and writing
 * it through the cast ran off the end of the array into whatever the linker
 * put next -- which is how three of those callbacks came to hold heap
 * addresses, and how a Classic run on the tablet ended up calling one of
 * them. Give it the type it is used as. */
static UnkAllstarData lbl_80472CB0_obj;
#define PC_ALLSTAR_DATA() (&lbl_80472CB0_obj)
#else
static u8 lbl_80472CB0[0x78];
#define PC_ALLSTAR_DATA() (PC_ALLSTAR_DATA())
#endif

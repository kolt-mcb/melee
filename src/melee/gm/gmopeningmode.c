...
void onExitTitle(GameModeState* arg0)
{
     ...
     * GM_OPENING_MV (the mode the port boots into), so it is the exit that
     * actually runs. Redirecting here rather than at boot reuses the opening
     * path's heap/arena warm-up, which the scene-heap setup in gm_801A4014
     * needs. Roadmap M2 uses 14 = GM_DEBUG_VS (menu-free VS on Final
     * Destination). */
    {
...
void gm_SetupTitleDemo(void)
{
    ...
    do {
        do {
             ...
             * characters unlocked the draw reads past the pool into the
             * stack. Draw from what there is. */
            j = pool[HSD_Randi(count < 8 ? (count > 0 ? count : 1) : 8)];
#else
            j = pool[HSD_Randi(8)];
#endif
            dup = 0;
    ...
    if (getenv("MELEE_MODELOG") != NULL) {
        fprintf(stderr, "[DEMO] pool %d unlocked; picks %d %d %d %d\n",
                (int) count, gm_801BF648(0), gm_801BF648(1), gm_801BF648(2),
                gm_801BF648(3));
    }
#endif
    ...
    do {
#if BUILD_TARGET_PC
        /* Same clamp as the character draw above: with fewer stages unlocked
         * the console's fixed eight reads past the pool, and the tablet's
         * stack said stage 0 -- no ground, one fighter at scale zero. */
        count = stage_pool[HSD_Randi(count < 8 ? (count > 0 ? count : 1) : 8)];
#else
...
#if BUILD_TARGET_PC
    if (getenv("MELEE_MODELOG") != NULL) {
        fprintf(stderr, "[DEMO] stage pick %d -> stage kind %d\n",
                (int) count, (int) gm_801641CC((u8) count));
    }
#endif
...
void gm_PreloadTitleDemo(void)
{
...
#if BUILD_TARGET_PC
    if (getenv("MELEE_MODELOG") != NULL) {
        fprintf(stderr, "[DEMO] cache chars %d %d %d %d stkind %d\n",
    ...
    if (getenv("MELEE_AXTRACE") != NULL) {
        fprintf(stderr,
                "[GM] scene banks: chars %d %d %d %d stage %d -> mask %llx\n",
                gm_801BF648(0), gm_801BF648(1), gm_801BF648(2),
                gm_801BF648(3), gm_801BF694(),
                (unsigned long long) temp_ret);
    }
#endif
...
void onEnterVs(GameModeState* arg0)
{
...
#if BUILD_TARGET_PC
    if (getenv("MELEE_MODELOG") != NULL) {
        fprintf(stderr, "[DEMO] vs setup: state id %d\n",
                (int) gm_GetCurrentSceneIndex());
    }
    /* PC port: guard against a NULL or still-unconverted enter-data pointer
     * from a game mode whose data never got set up. */
    if (md == NULL || (uintptr_t) md < 0x1000000ULL) {
        return;
    }
#endif
    ...
    if (getenv("MELEE_MODELOG") != NULL) {
        fprintf(stderr, "[DEMO] vs setup done: stage %d players %d %d %d %d\n",
                (int) md->rules.stkind, md->players[0].ckind,
                md->players[1].ckind, md->players[2].ckind,
                md->players[3].ckind);
    }
#endif
}

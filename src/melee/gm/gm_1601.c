...
#if BUILD_TARGET_PC
    /* PC port: ty/ (the trophy system) may be stubbed out, in which case
     * Toy_8030813C is a weak stub returning NULL and these become the literal
     * pointers 0x4 and 0x24. Both are handed to lbArchive_LoadSymbols as
     * filenames -- that is the `invalid basename ptr 0x4` seen on the Game
     * Over screen, and the SIGSEGV at address 0x4 inside libc right after it.
     * Return NULL so callers can tell "no trophy name" from a bogus one. */
    {
        ToyModelFile* rec = Toy_8030813C(var_r3);
        return rec != NULL ? rec->archive_name : NULL;
    }
#else
...
char* gm_80160564(CharacterKind ckind, GameModeKind mode)
{
    ...
    {
        ToyModelFile* rec = Toy_8030813C(var_r3);
        return rec != NULL ? rec->symbol_name : NULL;
    }
#else
...
void gm_80166378(MatchEnd* arg0_raw)
{
    ...
    if (!pc_ptr_sane(arg0_raw)) {
        PORT_LOG_WARN(
            "gm_80166378: invalid match-end struct %p; skipping standings\n",
            (void*) arg0_raw);
        return;
    }
#endif

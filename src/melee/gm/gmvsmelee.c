                        ...
                        CSSMatchType match_type)
{
    ...
    if (getenv("MELEE_CSSLOG") != NULL) {
        fprintf(stderr,
                "[CSS] vs->css copy: slot types %d %d %d %d match_type=%d\n",
                vs->start.players[0].slot_type, vs->start.players[1].slot_type,
                vs->start.players[2].slot_type, vs->start.players[3].slot_type,
                match_type);
    }
#endif

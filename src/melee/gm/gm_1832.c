...
void fn_801852FC(HSD_GObj* gobj, int pc_render_code)
{
    (void) pc_render_code;
    int i;
    int j;
...
#if BUILD_TARGET_PC
    /* PC port: GXWGFifo is the GameCube write-gather pipe, a hardware
     * address (0xCC008000). GXVert.h redirects only the GXPosition
     * wrappers for this target, so a raw `GXWGFifo.f32 =` store still goes
     * to that address and faults -- this is what killed Classic round 7,
     * SIGSEGV at 0xcc008000. The vertex descriptor here declares position
     * only, so the twelve stores are four 3-float positions; psdisp.c
...
void fn_8018564C(HSD_GObj* gobj, int pc_render_code)
{
    (void) pc_render_code;
...
void fn_8018569C(HSD_GObj* gobj, int pc_render_code)
{
    (void) pc_render_code;
    int i;
    PAD_STACK(8);
...
void fn_8018575C(HSD_GObj* gobj, int pc_render_code)
{
    (void) pc_render_code;
    if (HSD_CObjSetCurrent(GET_COBJ(gobj))) {
        Camera_800313E0(gobj, 1);
...
void fn_80186634(void* arg0)
{
    ...
    case 3:
        ...
        if ((s32) lbl_8047368C.model_scale_kind == 4) {
            GObj_SetupGXLinkMax(gobj3, fn_8018569C,
                                8);
        } else {
            GObj_SetupGXLinkMax(gobj3, fn_8018575C,
                                8);
        }
        fn_80186400();
        ...
        HSD_GObj_SetupProc(gobj4, fn_8018504C, 0x11);

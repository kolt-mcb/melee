 ...
 * defense ratio ~0 and no hit ever launched anyone.
 *
 ...
 *    byte view stays right;
 *  - x10: the per-round stage list and its five spawn entries.
...
#define PC_EV_MAX 64
static struct gm_804D6900_t pc_ev_lvls[PC_EV_MAX];
...
static struct gm_evstage_table pc_ev_x10s[PC_EV_MAX];
...
static void pc_gmevent_convert(void)
{
    ...
    if (raw_tbl == NULL ||
        (const void*) raw_tbl == (const void*) pc_ev_lvl_ptrs)
    {
        return;
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        const u8* lr;
        struct gm_804D6900_t* lvl = &pc_ev_lvls[i];
        u32 off;

        pc_ev_lvl_ptrs[i] = lvl;
        ...
        if (off != 0 && off + 0x28 <= len) {
            ...
            init->is_teams = r[2];
            init->item_freq = (s8) r[3];
            init->sd_penalty = (s8) r[4];
            init->unk5 = r[5];
            init->stkind = pc_ev_be16(r + 6);
            init->time_limit = pc_ev_be32(r + 8);
            /* +0x10..+0x17 is one 64-bit item mask. */
            init->x10 = ((u64) pc_ev_be32(r + 0x10) << 32) |
                        pc_ev_be32(r + 0x14);
        ...
        if (off != 0 && off + 0x30 <= len) {
            const u8* r = base + off;
            struct gm_evstage_table* x = &pc_ev_x10s[i];

            memset(x, 0, sizeof(*x));
            x->count = r[0];
            x->pad1 = r[1];
            for (j = 0; j < 7; j++) {
                x->stage[j] = pc_ev_be16(r + 2 + 2 * j);
            }
            for (j = 0; j < 5; j++) {
    ...
    if (getenv("MELEE_GMEV_TRACE") != NULL) {
        fprintf(stderr,
                ...
                "pinit0 x1C=%.2f)\n",
                n, pc_ev_lvls[0].kind,
                pc_ev_lvls[0].evinit ? (double) pc_ev_lvls[0].evinit->x1C : -1.0,
...
void gm_801BC670(HSD_GObj* arg0)
{
     ...
     * byte list); this reader wants two big-endian ints out of it. */
    {
        const u8* r = (const u8*) temp_r30;
        s32 v0 = r != NULL ? (s32) pc_ev_be32(r + 0) : 0;
        s32 v4 = r != NULL ? (s32) pc_ev_be32(r + 4) : 0;

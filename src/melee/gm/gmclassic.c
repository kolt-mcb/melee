...
#if BUILD_TARGET_PC
        /* A zeroed table never presents the sentinel, and this scan then
         * walks until it faults. The largest of the four tables this is
         * called with holds 39 entries. */
        if (count >= 39) {
            return;
        }
#endif
...
static gm_803DDEC8Struct* gmClassic_801B2D54(gm_803DDEC8Struct* arg0)
{
     ...
     * gm_Mode_Classic_States in .data there. GameModeState is 0x18 bytes on
     * GCN and 0x28 here, so states[26] spans 0x410 rather than 0x270 and that
     * pun lands on nothing. Address the table by name instead. */

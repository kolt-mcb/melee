    ...
    if (subject_count != 0) {
        ...
        while (subject != NULL) {
            if (Camera_8002928C(subject)) {
#if BUILD_TARGET_PC
                {
                    ...
                    if (on) {
                        fprintf(stderr,
                                ...
                                (unsigned) gm_GetFrameCount(), (int) subject_count,
                                *(u32*) &subject->pos.x,
                                *(u32*) &subject->pos.y,
                                *(u32*) &subject->ext.h.x,
                                *(u32*) &subject->ext.h.y,
                ...
                test_pos.x =
                    CM_FMA(subject->ext.h.x, tracking_multiplier, base_pos.x);
                ...
                test_pos.x =
                    CM_FMA(subject->ext.h.y, tracking_multiplier, base_pos.x);
                cam_bounds_flags = Camera_80029124(&test_pos, 0);
                if (cam_bounds_flags != CAM_BOUNDS_INSIDE) {
                ...
                test_pos.y =
                    CM_FMA(subject->ext.v.y, tracking_multiplier, base_pos.y);
                cam_bounds_flags = Camera_80029124(&test_pos, 0);
                if (cam_bounds_flags != CAM_BOUNDS_INSIDE) {
                ...
                test_pos.y =
                    CM_FMA(subject->ext.v.x, tracking_multiplier, base_pos.y);
                cam_bounds_flags = Camera_80029124(&test_pos, 0);
                if (cam_bounds_flags != CAM_BOUNDS_INSIDE) {
                ...
                if (getenv("MELEE_CAMTRACE_SUBJ") != NULL) {
                    ...
                    if (pc_frame_number >= 225 && n < 400) { n++;
                        ...
                        fprintf(stderr, "[CAMSOLVE] frame=%d subj=%p x10=(%.1f,%.1f) "
                                ...
                                (double) subject->pos.x, (double) subject->pos.y,
                                (double) subject->ext.h.x, (double) subject->ext.h.y,
                                (double) subject->ext.v.x, (double) subject->ext.v.y,
...
void Camera_80029CF8(CameraBounds* bounds, CameraTransformState* transform)
{
     ...
     * target it produced. The console's is at game_camera+0x14 (the
     ...
     * itself is a local on both sides, so it has to be printed. */
    {
        ...
        if (on) {
            ...
            fprintf(stderr,
                ...
                (unsigned) gm_GetFrameCount(), *(u32*) &bounds->x_min,
        ...
        if (!isfinite(tx) || !isfinite(ty)) { tx = 0.0f; ty = 0.0f; }
        {
            ...
            if (on) {
                fprintf(stderr,
                        ...
                        "t=(%08x,%08x)\n",
                        (unsigned) gm_GetFrameCount(), *(u32*) &input_x,
                        *(u32*) &input_y, *(u32*) &game_camera.quake_scale,
    ...
    if (var_r31 != 0) {
        ...
        if (getenv("MELEE_CAMCLAMP") != NULL) {
            ...
            fprintf(stderr,
                    ...
                    "hfov=%08x\n",
                    (unsigned) gm_GetFrameCount(), (int) var_r30, (int) var_r31,
...
void Camera_8002B0E0(void)
{
    ...
    if ((gm_IsCurrently1PMode_inline() != 0) && (game_camera.x2C0 > 0.0f)) {
        ...
        game_camera.x2BC = CM_FMA(-var_f2, cm_803BCCA0.xD4, game_camera.x2BC); /* fnmsubs 8002B1B4 */
                                    ...
                                    CameraTransformState* ts)
{
    ...
    ts->fov = CM_FMA(delta, cm_803BCCA0.x44, ts->fov); /* fmadds, both calls */
    ...
    if (getenv("MELEE_CAMTRACE") != NULL) {
        ...
        if ((n++ % every) == 0) {
            ...
            for (slot = 0; slot < 4; slot++) {
                ...
                if (s != NULL) {
                    fprintf(stderr,
                            ...
                            "x48=(%.1f,%.1f,%.1f)\n",
                            (int) pc_frame_number, slot, (double) s->pos.x,
                            (double) s->pos.y, (double) s->pos.z,
                            (double) s->bone_pos.x, (double) s->bone_pos.y,
                            (double) s->bone_pos.z, (double) s->target_ext.h.x,
                            (double) s->target_ext.h.y, (double) s->target_ext.v.x,
                            (double) s->target_ext.v.y, (double) s->target_ext.v.z);
                }
            }
            fprintf(stderr,
                    ...
                    (int) pc_frame_number,
                    (double) game_camera.transform.position.x,
                    (double) game_camera.transform.position.y,
                    (double) game_camera.transform.position.z,
                    (double) game_camera.transform.interest.x,
                    (double) game_camera.transform.interest.y,
                    (double) game_camera.transform.interest.z,
            ...
            fprintf(stderr,
                    ...
                    (double) game_camera.transform.target_position.x,
                    (double) game_camera.transform.target_position.y,
                    (double) game_camera.transform.target_position.z,
                    (double) game_camera.transform.target_interest.x,
                    (double) game_camera.transform.target_interest.y,
                    (double) game_camera.transform.target_interest.z,
                    (double) game_camera.transform.target_fov,
                    (double) game_camera.x2C0); }
    }
#endif
}
...
void Camera_8002C1A8(void)
{
    ...
    if (dir != 0) {
        scale = CM_FMA(game_camera.x32C, cm_803BCCA0.x8C, cm_803BCCA0.x90);
...
void Camera_8002C908(void* arg0)
{
    ...
    *copy_src = *subject_pos;
    ...
    game_camera.transform.fov =
        CM_FMA(delta, cm_803BCCA0.x88, game_camera.transform.fov); /* 8002CAE0 */
...
void Camera_8002CDDC(void* unused)
{
        ...
        ABS(subject->bone_pos.z) < 30.0f)
    {
        ...
        delta = game_camera.transform.target_fov - game_camera.transform.fov;
        game_camera.transform.fov =
            CM_FMA(delta, cm_803BCCA0.x70,
                   game_camera.transform.fov); /* 8002D06C.. */
        return;
    }
...
static inline void approach_vec3(Vec3* target, Vec3* cur, f32 smooth)
{
    ...
    z = target->z;
    iz = cur->z;
    /* One fmadds per axis, at every call. */
    cur->x = CM_FMA(x, smooth, ix);
    ix = z - iz;
    cur->y = CM_FMA(dy, smooth, cur->y);
    cur->z = CM_FMA(ix, smooth, cur->z);
}
...
void Camera_8002D318(void* unused)
{
        ...
        ABS(subject->bone_pos.z) < 10.0f)
    {
            ...
            (subject = ftLib_80086B74(gobj)) != NULL)
        {
            ...
            /* 8002D400: fnmsubs, pos.x - half_z*cos in one rounding. */
            game_camera.transform.target_interest.x =
                CM_FMA(-half_z, cosf(game_camera.yaw_offset), pos->x);
            game_camera.transform.target_interest.y = pos->y;
            game_camera.transform.target_interest.z =
                CM_FMA(half_z, sinf(game_camera.yaw_offset), pos->z); /* 8002D424 */
            approach_vec3(&game_camera.transform.target_interest,
                          &game_camera.transform.interest, cm_803BCCA0.x64);
        }

        game_camera.transform.target_fov = cm_803BCCA0.x6C;
        delta = game_camera.transform.target_fov - game_camera.transform.fov;
        game_camera.transform.fov = CM_FMA(delta, cm_803BCCA0.x70,
                                           game_camera.transform.fov); /* fmadds */
        ...
        /* One fmadds per axis (8002D52C-8002D560, and again in 8002D85C). */
        game_camera.transform.target_position.x =
            CM_FMA(horiz_dist, sinf(game_camera.yaw_offset),
                   game_camera.transform.target_interest.x);
        game_camera.transform.target_position.y =
            CM_FMA(distance, sinf(game_camera.pitch_offset),
                   game_camera.transform.target_interest.y);
        game_camera.transform.target_position.z =
            CM_FMA(horiz_dist, cosf(game_camera.yaw_offset),
                   game_camera.transform.target_interest.z);
        approach_vec3(&game_camera.transform.target_position,
                      &game_camera.transform.position, cm_803BCCA0.x68);
        return;
    }
                             ...
                             CameraUnkGlobals* globals)
{
    ...
    cam->transform_copy.interest.x =
        CM_FMA(dx, globals->x74, cam->transform_copy.interest.x);
    cam->transform_copy.interest.y =
        CM_FMA(dy, globals->x74, cam->transform_copy.interest.y);
    cam->transform_copy.interest.z =
        CM_FMA(dz, globals->x74, cam->transform_copy.interest.z);
}
...
void Camera_8002DDC4(void* unused)
{
    ...
    /* 8002DE18..: every lerp here is one fmadds of delta*smooth onto the
     * current value; the fov steps likewise (8002DEB4, 8002DEDC). */
    {
        ...
        cam->transform.interest.x = CM_FMA(dx, smooth, cam->transform.interest.x);
        cam->transform.interest.y = CM_FMA(dy, smooth, cam->transform.interest.y);
        cam->transform.interest.z = CM_FMA(dz, smooth, cam->transform.interest.z);
    }
    ...
    cam->transform.target_fov = Stage_GetCamFixedFov();
    {
        ...
        cam->transform.fov = CM_FMA(delta, globals->x7C, cam->transform.fov);
    }
    ...
    target_position = &transform->target_position;
    {
        ...
        cam->transform_copy.fov = CM_FMA(delta, globals->x7C, cam->transform_copy.fov);
    }
    Stage_SetVecToFixedCamPos(target_position);
    {
        ...
        cam->transform.position.x = CM_FMA(dx, smooth, cam->transform.position.x);
        cam->transform.position.y = CM_FMA(dy, smooth, cam->transform.position.y);
        cam->transform.position.z = CM_FMA(dz, smooth, cam->transform.position.z);
    }
    Stage_SetVecToFixedCamPos(&transform_copy->target_position);
    {
        ...
        cam->transform_copy.position.x = CM_FMA(dx, smooth, cam->transform_copy.position.x);
        cam->transform_copy.position.y = CM_FMA(dy, smooth, cam->transform_copy.position.y);
        cam->transform_copy.position.z = CM_FMA(dz, smooth, cam->transform_copy.position.z);
    }
...
void Camera_8002E490(void* unused)
{
    ...
    case 1:
        ...
        break;
    case 2:
        game_camera.x378.f32_v = CM_FMA(1.0f - game_camera.x378.f32_v,
                                      game_camera.x37C.f32_v,
                                      game_camera.x378.f32_v); /* 8002E6A0 */
...
void Camera_8002F7AC(s8 slot)
{
    ...
    if (fighter_gobj != NULL) {
        ...
        if (cam_box != NULL) {
            ...
            game_camera.pitch_offset = CM_FMAD(M_PI / 8, randf, -(M_PI / 16));
            return;
        }
    }
    // random pitch and yaw if no player or cambox
    game_camera.pitch_offset = CM_FMAD(M_PI / 8, HSD_Randf(), -(M_PI / 16));
    randf = HSD_Randf();
    game_camera.yaw_offset = CM_FMAD(M_TAU, randf, -M_PI); /* 8002F8D0 */
}
...
void Camera_8002FC7C(s8 arg0, s8 arg1)
{
    ...
    game_camera.x2D0.angle_right = Stage_GetCamAngleRadiansRight();
    game_camera.x2D0.angle_left = Stage_GetCamAngleRadiansLeft();
    temp_f2 = CM_FMA(game_camera.x32C, (*new_var).x8C, (*new_var).x90);
    ...
    for (slot = 0; slot < 4; slot++) {
        ...
        if (g == NULL) continue;
        {
            ...
            fprintf(stderr, "[FTPOS] slot=%d gobj=%p x1C=(%.1f,%.1f,%.1f)\n",
                    slot, (void*) g, (double) s->bone_pos.x, (double) s->bone_pos.y,
                    (double) s->bone_pos.z);
        }
    }
}
...
static inline void project_ground_x(f32* value, Vec3* eye_pos, f32 dir_x,
                                    f32 dir_z)
{
    /* fnmsubs at each of its three calls (800308D8, 80030948, 800309E4):
     * eye.x - dir_x * (eye.z / dir_z) in one rounding. */
    *value = CM_FMA(-dir_x, eye_pos->z / dir_z, eye_pos->x);
}
...
bool Camera_800307D0(f32* left, f32* center, f32* right)
{
    ...
    if (ABS(forward.x) > 1E-4L && ABS(forward.z) > 1E-4L) {
        ...
        s = sinf(half_fov);
        c = cosf(half_fov);
        /* 80030904 / 80030908: the c products are plain fmuls; the s
         * products fuse onto them -- fmadds for x, and for z an fmsubs of
         * fx*s - fz*c that the code then negates, which fma(-fx, s, fz*c)
         * is exactly. */
        edge_x = CM_FMA(forward.z, s, forward.x * c);
        edge_z = CM_FMA(-forward.x, s, forward.z * c);
        ...
        s2 = sinf(-half_fov);
        c2 = cosf(-half_fov);
                edge_x2 = CM_FMA(forward.z, s2, forward.x * c2); /* 800309A0/A4 */
        edge_z2 = CM_FMA(-forward.x, s2, forward.z * c2);
 ...
 * on, and the CObj is a frame behind it. game_camera is static, so the trace
...
void pc_camera_transform(float* out14)
{
    ...
    memcpy(out14, &game_camera.transform, 14 * sizeof(float));
}
#endif
...
bool Camera_80030BBC(Vec3* arg0, S32Vec2* arg1)
{
     ...
     * else says which. */
    {
        ...
        if (want >= 0 && (int) gm_GetFrameCount() == want) {

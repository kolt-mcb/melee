...
void ftKb_PkSpecialN_Anim(Fighter_GObj* gobj)
{
    ...
    if (fp->cmd_vars[ftPk_SpecialN_Cmd0] == true) {
        ...
        if (!fp->cmd_vars[ftPk_SpecialN_Cmd1]) {
            ...
            switch (fp->u.kb.hat.kind) {
            case FTKIND_PIKACHU:
                it_pos.x = KK_FMA(fp->x34_scale.y,
                                  da->specialn_pk_spawn_offset.x * fp->facing_dir,
                                  fp->cur_pos.x);
                it_pos.y = KK_FMA(da->specialn_pk_spawn_offset.y, fp->x34_scale.y,
                                  fp->cur_pos.y);
                it_pos.z = 0.0f;
            ...
            case FTKIND_PICHU:
                it_pos.x = KK_FMA(fp->x34_scale.y,
                                  da->specialn_pc_spawn_offset.x * fp->facing_dir,
                                  fp->cur_pos.x);
                it_pos.y = KK_FMA(da->specialn_pc_spawn_offset.y, fp->x34_scale.y,
                                  fp->cur_pos.y);
                it_pos.z = 0.0f;
                itPikachuThunderJolt_Spawn(gobj, &it_pos, fp->facing_dir,
        ...
        if (!fp->cmd_vars[ftPk_SpecialN_Cmd1]) {
            ...
            case FTKIND_PIKACHU:
                it_pos.x = KK_FMA(fp->x34_scale.y,
                                  ...
                                  fp->cur_pos.x);
                it_pos.y = KK_FMA(da->specialairn_pk_spawn_offset.y, fp->x34_scale.y,
                                  fp->cur_pos.y);
                it_pos.z = 0.0f;
                itPikachuThunderJolt_Spawn(gobj, &it_pos, fp->facing_dir,
                                           da->specialn_pk_itkind);
            ...
            case FTKIND_PICHU:
                it_pos.x = KK_FMA(fp->x34_scale.y,
                                  ...
                                  fp->cur_pos.x);
                it_pos.y = KK_FMA(da->specialairn_pc_spawn_offset.y, fp->x34_scale.y,
                                  fp->cur_pos.y);
                it_pos.z = 0.0f;
                itPikachuThunderJolt_Spawn(gobj, &it_pos, fp->facing_dir,
                                           da->specialn_pc_itkind);

#if BUILD_TARGET_PC
#include <math.h>
/* The CPU's attack choice is four margins compared against zero, each a
 * prediction of where the target will be in list->x04 frames. MWCC fused every
 * multiply-and-add in them -- including the ones that mix single and double,
 * where the half-gravity term comes off in a single fnmsub -- and a margin
 * near zero is exactly where an extra rounding decides differently from the
 * console. That shows up as one extra HSD_Randf, and from there the two
 * matches are different matches. The same three functions here all share the
 * shape, and so does their disassembly. */
#define CPU_FMA(a, b, c) fmaf((a), (b), (c))
#define CPU_FMAD(a, b, c) fma((a), (b), (c))
#else
#define CPU_FMA(a, b, c) ((a) * (b) + (c))
#define CPU_FMAD(a, b, c) ((a) * (b) + (c))
#endif
...
int ftCo_800B4AB0(Fighter* fp, Fighter* target, void* arg2)
{
    ...
    cpu->x6C.y = 0.0f;
    cpu->x6C.x = 0.0f;
    halfRange = (f32) CPU_FMAD(0.5, cpu->x570, 0.5); /* 800B4B34 */
    if (list == NULL) {
        return 0;
    }
    ...
    while (list->cmd) {
        ...
        t = list->x04;
        relx = CPU_FMA(tgtVx, t, tgtX) -
               CPU_FMA(fpVx, t, fpX); /* 800B4C60, 800B4C64 */
        if (target->ground_or_air == GA_Air) {
            ...
            if (v <= 0.0f) {
                fpPredY = CPU_FMA(fpVy, t, fpY);
            } else if (t < v) {
                sq = sqrtf_store(t, &sqrt_tmp[2]);
                /* 800B4D18: fnmsub, so the half-gravity term comes off
                 * the velocity term in one rounding, in double. */
                fpPredY = (f32) ((f64) fpY +
                                 CPU_FMAD(-0.5, (f64) (fpGrav * sq),
                                          (f64) (fpVy * t)));
            } else {
        ...
        if (target->ground_or_air == GA_Air) {
            ...
            if (v <= 0.0f) {
                relPredY = CPU_FMA(tgtVy, t, tgtY) - fpPredY;
            } else if (t < v) {
                sq = sqrtf_store(t, &sqrt_tmp[0]);
                /* 800B4E58 then 800B4E5C: the inner sum is a
                 * single-precision fmadds and the half-gravity term comes
                 * off it in double. */
                relPredY = (f32) (CPU_FMAD(-0.5, (f64) (tgtGrav * sq),
                                           (f64) CPU_FMA(tgtVy, t, tgtY)) -
                                  (f64) fpPredY);
            } else {
                sq = sqrtf_store(v, sqrt_tmp - 1);
                relPredY = (f32) (((f64) (tgtTermNeg * (t - v)) +
                                   CPU_FMAD(-0.5, (f64) (tgtGrav * sq),
                                            (f64) CPU_FMA(tgtVy, t, tgtY))) -
                                  (f64) fpPredY);
            }
        } else {

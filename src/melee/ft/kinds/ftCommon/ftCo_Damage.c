        ...
        if (fp->ground_or_air == GA_Air) {
...
#if BUILD_TARGET_PC
            /* MELEE_KBDBG=1: the knockback vector a hit or throw release
             * produces and the two multipliers behind it. The console gives
             * the same from a code breakpoint at ftCo_Damage_CalcVel, whose
             * f1/f2 are x and y. */
            if (getenv("MELEE_KBDBG") != NULL) {
                ...
                fprintf(stderr,
                        ...
                        (unsigned) gm_GetFrameCount(), *(u32*) &kb_applied,
...
void ftCo_8008E5A4(Fighter* fp)
{
    if (fp->input.lstick[0].x || fp->input.lstick[0].y) {
        ...
        if (!(kb_mag < 0.00001f)) {
             ...
             * register holds. */
            float f3 = DMG_FMA(kb_y, fp->input.lstick[0].x,

...
static HSD_Joint* pc_guard_joint(Fighter* fp)
{
    ...
    return fp->ft_data->x20->x0[2];
}
#define PC_GUARD_JOINT(fp) pc_guard_joint(fp)
#else
#define PC_GUARD_JOINT(fp) ((fp)->ft_data->x20->x0[2])
#endif
        ...
        if (fp->mv.co.guard.x4) {
            ...
            if (fp->mv.co.guard.x4 < 1) {
                ftAnim_80070108(fp, FtPart_TransN, 1 - fp->mv.co.guard.x4,
                                fp->mv.co.guard.x4, PC_GUARD_JOINT(fp));
            }
            if (arg1 < 1) {
            ...
            ftAnim_8006FA58(fp, FtPart_TransN, PC_GUARD_JOINT(fp));
        }
        {
            scl.x = scl.y = scl.z = inlineB0(fp);
            HSD_JObjSetScale(fp->parts[fp->ft_data->x8->x11].joint, &scl);
...
void ftCo_80093240(Fighter_GObj* gobj)
{
    ...
    if (fp->allow_sdi && fp->ground_or_air == GA_Ground) {
            ...
            fp->active_timer.lstick.x < p_ftCommonData->sdi_stick_window)
        {
            ...
            fp->cur_pos.x =
                GD_FMA(fp->coll_data.floor.normal.y, scl, fp->cur_pos.x);
            fp->cur_pos.y =
                GD_FMA(-fp->coll_data.floor.normal.x, scl, fp->cur_pos.y);

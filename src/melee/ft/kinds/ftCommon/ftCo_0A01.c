...
void ftCo_800A101C(Fighter* arg0, int arg1, int arg2, int arg3)
{
    ...
    if (getenv("MELEE_CPUATK") != NULL) {
        ...
        fprintf(stderr, "[CPUATK] gframe=%u p%d INIT type=%d level=%d\n",
                (unsigned) gm_GetFrameCount(), (int) arg0->player_id, arg1,
                arg2);
        ...
        if (dx > 0.0f) {
            ...
            if (arg1 != 0) {
                ...
                if (ok != 0) {
                    ...
                    PAD_STACK(8);
                    /* fmadds: ex*ex takes the fsubs'd ddy*ddy on. */
                    dist = sqrtf_store(CO_FMA(ex, ex, ddy * ddy),
                                       &sqrt_dist_store);
                    if (data->x5C > dist) {
                        ftCo_800A75DC_set_target(fp, &fp->cpu.x60, px, ey,
                                                 5.0f);
...
Fighter* ftCo_800A4BEC(Fighter* fp)
{
    ...
    if (getenv("MELEE_TGT") != NULL) {
        extern u32 gm_GetFrameCount(void);
        HSD_GObj* c2;
        for (c2 = HSD_GObjPLinkHead[HSD_GOBJ_PLINK_FIGHTER]; c2 != NULL; c2 = c2->next) {
            Fighter* o;
            if (fp->gobj == c2) {
                continue;
            }
        ...
        if (!ftCo_IsNearlyZero(kb_mag)) {
            ...
            if (getenv("MELEE_DIDBG") != NULL) {
                ...
                fprintf(stderr,
                        ...
                        (unsigned) gm_GetFrameCount(), (int) fp->player_id, bx.u,
    ...
    if (data->csP == NULL && data->command_duration == 0) {
        ...
        if (getenv("MELEE_AIDISP") != NULL) {
            ...
            fprintf(stderr, "[AIDISP] gframe=%u p%d x18=%d x1C=%d\n",
                    (unsigned) gm_GetFrameCount(), (int) fp->player_id, (int) cmd,
                    (int) data->x1C);
        }
#endif
        {

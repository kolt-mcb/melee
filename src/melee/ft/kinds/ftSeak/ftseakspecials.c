...
bool ftSk_SpecialS_CheckInitChain(HSD_GObj* gobj)
{
    ...
    if (fp->mv.sk.specials.x0 == da->x1C + 1) {
        ...
        HSD_GObj* item_gobj = fp->u.sk.x8;
#if BUILD_TARGET_PC
        /* PC port: the chain item can be absent -- this file already
         * NULL-checks fp->u.sk.x8 at its other three uses; this one
         * dereferenced it unguarded and faulted at HSD_GObj::user_data
         * (0x48). Reached once Classic runs at a difficulty where Sheik
         * actually uses the chain. */
        if (item_gobj != NULL)
#endif
        {

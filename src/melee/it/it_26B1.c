...
/* PC port: the console's sqrt here is the frsqrte Newton iteration below,
 * not libm's exactly-rounded sqrtf; keep it on the host so the arithmetic
 * agrees. */
#if MUST_MATCH || BUILD_TARGET_PC
...
float it_8026B424(int damage)
{
    ...
    return (int) IT_FMA((f32) damage, itCommonData->xB8, itCommonData->xBC);
}

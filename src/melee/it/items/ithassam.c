...
bool itHassam_UnkMotion1_Anim(Item_GObj* gobj)
{
    ...
    if (ip->xDB0_itcmd_var1 != 0) {
        ...
        /* One fmadds on the console: MTXDegToRad's multiply and the add.
         * MTXDegToRad(1.0f) folds to the macro's own constant. */
        ip->xDD4_itemVar.hassam.x68 = HS_FMA(
            MTXDegToRad(1.0f), 180 / attr->x20, ip->xDD4_itemVar.hassam.x68);

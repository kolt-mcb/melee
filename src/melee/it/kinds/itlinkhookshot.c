...
static inline f32 it_802A4BFC_sqrtf_offset(f32 x)
{
    ...
    if (x > 0.0F) {
        ...
        guess = 0.5 * guess * (3.0 - guess * guess * x);
#if BUILD_TARGET_PC
        /* PC port: the retail spill slot is 6 floats past `y`, which is a
         * stack write past the local on the host. Use the local itself; the
         * value is the same, only the slot it lands in differs. */
        y = (f32) (x * guess);
        return y;
#else
        *(&y + 6) = (f32) (x * guess);
                 ...
                 Fighter* arg3)
{
    ...
    while (link_1 != NULL) {
        ...
        if (len > attr->x30) {
            ...
            link_1->pos.y = LK_FMA(vec.y, attr->x30, cur->pos.y);
            {
                f32 z = LK_FMA(vec.z, attr->x30, cur->pos.z); // permuterslop
                (void) z;
                link_1->pos.z = z;
            }
        }
                 ...
                 itLinkHookshotAttributes* attrs)
{
    ...
    while (next != NULL && next->x2C_b0) {
        if (!retracted) {
            if (it_802A6A78_normalize_diff(&next->pos, &cur->pos, &dir) >
                attrs->x30)
            {
                next->pos.x = LK_FMA(dir.x, attrs->x30, cur->pos.x);
                next->pos.y = LK_FMA(dir.y, attrs->x30, cur->pos.y);
                next->pos.z = LK_FMA(dir.z, attrs->x30, cur->pos.z);
            } else {
                retracted = true;
            }
        }
    ...
    if (it_802A6A78_normalize_diff(pos, &cur->pos, &dir) > attrs->x30) {
        pos->x = LK_FMA(dir.x, attrs->x30, cur->pos.x);
        pos->y = LK_FMA(dir.y, attrs->x30, cur->pos.y);
        pos->z = LK_FMA(dir.z, attrs->x30, cur->pos.z);
    }
}
